function TreeView(datas, options) {
        ...
        for (var index = 1; index < $parents.length; index++) {
            ...
            for (var i = 0; i < $children.length; i++) {
                if ($children[i].checked != 1)
                    all1 = false;
                if ($children[i].checked != 0)
                    all0 = false;
            }
            if (all1)
                setCheckState.bind(item)(1);
            else if (all0)
                setCheckState.bind(item)(0);
            else
                setCheckState.bind(item)(2);
        }
    }
    ...
    function setCheckState(value) {
        ...
        if (value == 1) {
            $(this).find(">[check-icon]")[0].className = "far fa-check-circle";
        }
        if (value == 2) {
            $(this).find(">[check-icon]")[0].className = "far fa-dot-circle";
        }
    }
    ...
    function createSingleItem(data) {
        ...
        /* if ("fold" in options) {
         ...
         foldButton.className = "fas fa-angle-right rotate";
